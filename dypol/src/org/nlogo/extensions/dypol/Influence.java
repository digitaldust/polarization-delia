...
class Influence extends DefaultCommand {
    ...
    public void perform(Argument[] argmnts, Context cntxt) throws ExtensionException, LogoException {
        ...
        for (Node pi : turtles) {
            ...
            Node[] contacts = sample(turtles, .005 * Math.pow(Math.round(issuesMean), 2));
            if(pi.getContactedPeople(ticks)==null){
                Double[] contactedPeople = new Double[Dypol.size];
            ...
            for (Node pj : contacts) {
                double nextBinomial = Dypol.generator.nextBinomial(pi.getSocDistanceProb(ticks)[pj.getWho()], 1);
                pi.getContactedPeople(ticks)[pj.getWho()] = nextBinomial;
        ...
        for (Node pi : turtles) {
            for (Node pj : turtles) {
                if (pi.getContactedPeople(ticks)[pj.getWho()] == 1) {
                    ...
                    // TODO : add a link in the NetLogo world and update network layout
    ...
    private Node[] sample(Collection<Node> turtles, double par) {
        Node[] sample = new Node[(int) par];
        Object[] pool = turtles.toArray();
        Collections.shuffle(Arrays.asList(pool));
        for (int i = 0; i < sample.length; i++) {
            sample[i] = (Node) pool[i];
        }
        return sample;
    }
}
